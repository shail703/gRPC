...
def start_server_script(port):
    # Start the subprocess and store the Popen
    global process
    process = subprocess.Popen(["python", "server_script.py", str(port)])


def stop_server_script():
    global process
    if process:
...
class Node:
    ...
    def startHeartBeat(self):
        ...
        print(port)
        # process = subprocess.Popen(["python3", "server_script.py", port])
        start_server_script(port)
    ...
    def reset_timeout(self):
        self.election_time = time.time() + cfg.LEADER_TIMEOUT + random.uniform(0, 0.15)

    # /heartbeat

    def heartbeat_follower(self, msg):
        term = msg["term"]
        if self.term < term or (self.term == term and self.status != LEADER):
            print(
                f"Node {self.addr} stepping down to FOLLOWER from {self.status} in term {self.term}. New leader: {msg['addr']}"
            )
    ...
    def timeout_loop(self):
        ...
        while self.status != LEADER:
            ...
            if delta < 0:
                # print(f"Leader timeout for {self.addr}. Starting new election.")
                # stop_server_script()
                self.startElection()
            else:
                time.sleep(delta)
